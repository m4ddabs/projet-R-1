...
Nous ajoutons une variable (ETP_tot) contenant le total des ETP pour chaque spécialité,  à la table specialites.
```{r}
...
Dans cette partie de l'analyse, nous visons à créer une ligne d'approximation linéaire avec un intervalle de confiance associé afin de mettre en relation le nombre nécessaire de  médecins oncologues ETP en fonction du nombre de Patients pour d'éventuels nouveaux hôpitaux
...
Pour calculer les intervalles de confiance pour la régression linéaire, nous supposons que les bruits suivent (approximativement) une distribution gaussienne. Cette hypothèse est valable en raison du théorème centrale de limite et lorsque le nombre d'échantillons est grand. Nous représentons ici graphiquement la régression linéaire avec le log-ajustement et sans, représentant l'intervalle de confiance
```{r}
#remove Na values
...
```{r}
plot(acp_specialites,choix="var",axes=c(1,2))
```
